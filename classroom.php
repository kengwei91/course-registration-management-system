...
if(isset($_POST['register'])){
	if(isset($_POST['class_id']) AND isset($_POST['course_id']) AND isset($_POST['course_price'])){
		...
		echo insertOrders($classid,$courseid,$sid,$courseprice);
		}else{
...
if(isset($_POST['waitlist'])){
	if(isset($_POST['class_id']) AND isset($_POST['course_id'])){
		...
		//if specific waiting list added, then won't add to waiting list again.
		$sql = 'SELECT waitinglist.ClassID, SID, Classroom.ClassDate
				FROM waitinglist
				INNER JOIN Classroom ON waitinglist.ClassID = Classroom.ClassID
				WHERE waitinglist.ClassID = '.$classid.' AND SID = '.$sid.' AND Classroom.ClassDate = "'.$rowdate['ClassDate'].'";';
		$result = mysqli_query($conn, $sql);
		...
		if(!mysqli_num_rows($result) > 0){
			...
			if(!mysqli_num_rows($result) > 0){
				echo insertWaitList($classid,$courseid,$sid);
			}else{
				$msg = '<div class="alert alert-warning">
						<strong>Warning!</strong> You have already registered this class!
						</div>';
			...
			$msg = '<div class="alert alert-warning">
					<strong>Warning!</strong> You have already added into waiting list!
					</div>';
			echo $msg;
		}
	}
